...
static float toterr(const float *const tg, const float *const o,
                    const int size) {
  float sum = 0.0f;
...
static void bprop(const Tinn t, const float *const in, const float *const tg,
                  float rate) {
...
// Trains a tinn with an input and target output with a learning rate. Returns
// target to output error.
float xttrain(const Tinn t, const float *const in, const float *const tg,
              float rate) {
  fprop(t, in);
  bprop(t, in, tg, rate);
...
// Constructs a tinn with number of inputs, number of hidden neurons, and number
// of outputs
